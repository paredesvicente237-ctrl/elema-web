...
export function Header() {
  ...
  useEffect(() => {
    const updateCount = () => {
      ...
      setCartCount(parsed.reduce((sum: number, item: { quantity: number }) => sum + item.quantity, 0));
    };
    ...
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled || pathname !== '/' ? 'border-b border-white/10 bg-[#0b0d12]/90 shadow-[0_10px_45px_rgba(0,0,0,0.24)] backdrop-blur-xl' : 'bg-transparent'}`}>
      <div className="relative mx-auto flex h-20 max-w-7xl items-center px-4 sm:px-6 lg:px-8">
        <nav className="hidden flex-1 items-center gap-6 text-[0.72rem] uppercase tracking-[0.32em] text-[#d7d0c7] lg:flex">
          {leftItems.map((item) => (
            <Link key={item.href} href={item.href} className="transition hover:text-[#f4efe6]">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="ml-auto hidden flex-1 items-center justify-end gap-3 lg:flex">
          {rightItems.map((item) => (
            <Link key={item.href} href={item.href} className="text-[0.72rem] uppercase tracking-[0.32em] text-[#d7d0c7] transition hover:text-[#f4efe6]">
              {item.label}
            </Link>
          ))}
        ...
        <div className="ml-auto flex items-center gap-2 sm:gap-3 lg:hidden">
          ...
          <button className="rounded-full border border-white/10 p-2.5 text-[#d7d0c7] lg:hidden" aria-label="Abrir menú" onClick={() => setOpen((value) => !value)}>
      ...
      {open ? (
        <div className="border-t border-white/10 bg-[#0b0d12]/95 px-4 py-5 lg:hidden">
          <div className="flex flex-col gap-3 text-[0.72rem] uppercase tracking-[0.3em] text-[#d7d0c7]">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="transition hover:text-[#f4efe6]" onClick={() => setOpen(false)}>
                {item.label}
              </Link>
            ))}
