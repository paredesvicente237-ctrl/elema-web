...
export const metadata: Metadata = { title: 'Mi ELEM', robots: { index: false, follow: false } };
...
export default async function AccountPage() {
  ...
  if (!user) redirect('/ingresar?next=/mi-elem');
    ...
    <main className="min-h-screen bg-[#eee8de] px-4 pb-24 pt-28 text-[#171717] sm:px-6 lg:pt-36">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-6 border-b border-black/15 pb-8 sm:flex-row sm:items-end">
          <div><p className="text-[0.68rem] uppercase tracking-[0.34em] text-[#77716a]">Tu espacio ELEM</p><h1 className="mt-4 font-serif text-5xl sm:text-6xl">Hola, {firstName}.</h1><p className="mt-3 text-sm text-[#69635d]">Revisa los elementos de tus pedidos y el avance de cada conjunto.</p></div>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1.35fr_0.65fr]">
          <section>
            ...
            {orders.length ? <div className="mt-6 divide-y divide-black/10 border-y border-black/10 bg-[#f8f5ef]">{orders.map((order) => { const status = orderStatuses[order.status] ?? orderStatuses.pending_confirmation; return <Link key={order.id} href={`/mi-elem/pedidos/${order.id}`} className="group grid gap-4 p-5 transition hover:bg-white sm:grid-cols-[1fr_auto_auto] sm:items-center sm:p-6"><div><p className="font-medium">{order.order_number}</p><p className="mt-1 text-xs text-[#77716a]">{formatOrderDate(order.created_at)}</p></div><div className="sm:text-right"><p className="text-xs uppercase tracking-[0.15em] text-[#69635d]">{status.label}</p><p className="mt-1 font-serif text-xl">{formatClp(order.total)}</p></div><ArrowRight size={17} className="text-[#77716a] transition-transform group-hover:translate-x-1" /></Link>; })}</div> : <div className="mt-6 border border-dashed border-black/20 bg-[#f8f5ef] p-8 text-center sm:p-12"><Package size={25} className="mx-auto text-[#77716a]" /><h3 className="mt-5 font-serif text-3xl">Aún no tienes elementos en seguimiento.</h3><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#69635d]">Cuando confirmes una selección, podrás seguir cada etapa del conjunto desde aquí.</p><Link href="/tienda" className="mt-6 inline-flex bg-[#171717] px-5 py-3 text-xs uppercase tracking-[0.2em] text-white">Explorar elementos</Link></div>}
          </section>
          <aside className="space-y-5"><MarketingPreference initialValue={profile?.marketing_opt_in ?? false} /><section className="border border-black/10 bg-[#171717] p-6 text-white sm:p-8"><p className="text-[0.65rem] uppercase tracking-[0.28em] text-white/50">Un elemento por resolver</p><h2 className="mt-4 font-serif text-3xl">Estamos para ayudarte.</h2><p className="mt-3 text-sm leading-6 text-white/65">Habla directamente con ELEM si necesitas coordinar una entrega o resolver cualquier parte de tu pedido.</p><Link href="/contacto" className="mt-6 inline-flex items-center gap-2 text-xs uppercase tracking-[0.18em]">Contactar <ArrowRight size={14} /></Link></section></aside>
        </div>
      </div>
    </main>
  );
...
function AccountSetup() { return <main className="grid min-h-screen place-items-center bg-[#eee8de] px-4 pb-20 pt-28 text-[#171717]"><section className="max-w-xl border border-black/10 bg-[#f8f5ef] p-8 text-center shadow-xl sm:p-12"><p className="text-[0.68rem] uppercase tracking-[0.3em] text-[#77716a]">Mi ELEM</p><h1 className="mt-5 font-serif text-5xl">Portal preparado.</h1><p className="mt-5 text-sm leading-7 text-[#69635d]">La interfaz y la seguridad ya están implementadas. Para habilitar cuentas reales falta conectar el proyecto de Supabase con las variables de entorno.</p><Link href="/" className="mt-7 inline-flex bg-[#171717] px-5 py-3 text-xs uppercase tracking-[0.2em] text-white">Volver al inicio</Link></section></main>; }
