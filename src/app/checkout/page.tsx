    ...
    <main className="min-h-screen bg-[#eee8de] px-4 pb-24 pt-28 text-[#171717] sm:px-6 lg:px-8 lg:pt-36">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[0.85fr_1.15fr]">
          <section><p className="text-[0.7rem] uppercase tracking-[0.35em] text-[#77716a]">Resumen</p><h1 className="mt-4 font-serif text-5xl sm:text-6xl">Finaliza tu pedido.</h1><p className="mt-5 text-sm leading-7 text-[#69635d]">ELEMA confirmará disponibilidad, despacho y forma de pago directamente contigo.</p><div className="mt-8 divide-y divide-black/10 border-y border-black/10">{items.map((item) => <div key={item.id} className="flex justify-between gap-4 py-5 text-sm"><span>{item.name} × {item.quantity}</span><span>{formatClp((item.price ?? 0) * item.quantity)}</span></div>)}</div><div className="flex justify-between py-5 font-serif text-2xl"><span>Subtotal</span><span>{formatClp(subtotal)}</span></div><p className="text-xs leading-6 text-[#77716a]">El despacho no está incluido y se cotiza según destino y condiciones de acceso.</p><div className="mt-7 flex items-start gap-3 border-t border-black/10 pt-6 text-xs leading-6 text-[#69635d]"><LockKeyhole size={17} className="mt-0.5 shrink-0" /><p>Tu pedido se asociará a una cuenta privada para que solo tú puedas consultar sus datos y seguimiento.</p></div></section>
          <form onSubmit={sendOrder} className="border border-black/10 bg-[#f8f5ef] p-6 shadow-[0_25px_70px_rgba(20,16,10,0.08)] sm:p-8"><h2 className="font-serif text-3xl">Contacto y entrega</h2><div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Nombre" name="nombre" autoComplete="given-name" required /><Field label="Apellido" name="apellido" autoComplete="family-name" required /><Field label="Correo de tu cuenta" name="correo" type="email" autoComplete="email" required /><Field label="Teléfono" name="telefono" type="tel" autoComplete="tel" required /><Field label="Región" name="region" autoComplete="address-level1" required /><Field label="Comuna" name="comuna" autoComplete="address-level2" required /></div><div className="mt-5"><Field label="Dirección de entrega" name="direccion" autoComplete="street-address" required /></div><label className="mt-5 block text-sm text-[#4e4943]">Notas del pedido<textarea name="notas" maxLength={800} className="mt-2 min-h-28 w-full border border-black/15 bg-white px-4 py-3 text-[#171717] outline-none focus:border-black/50" /></label><label className="mt-5 flex items-start gap-3 text-xs leading-6 text-[#69635d]"><input type="checkbox" required className="mt-1 accent-[#171717]" /><span>Acepto la <Link href="/privacidad" className="underline">política de privacidad</Link> y que ELEMA me contacte para coordinar este pedido.</span></label><label className="mt-4 flex items-start gap-3 text-xs leading-6 text-[#69635d]"><input name="marketing" type="checkbox" className="mt-1 accent-[#171717]" /><span>Quiero recibir por correo novedades, lanzamientos y beneficios de ELEMA. Es opcional y puedo cancelar cuando quiera.</span></label>{error ? <div role="alert" className="mt-5 border border-red-800/20 bg-red-50 p-4 text-sm text-red-900"><p>{error}</p>{needsLogin ? <Link href="/ingresar?next=/checkout" className="mt-3 inline-flex font-medium underline underline-offset-4">Ingresar o crear cuenta</Link> : null}</div> : null}<button disabled={loading || !ready} type="submit" className="mt-7 inline-flex w-full items-center justify-center gap-2 bg-[#171717] px-5 py-4 text-xs uppercase tracking-[0.2em] text-white transition hover:bg-black disabled:cursor-wait disabled:opacity-60">{loading ? 'Guardando pedido…' : 'Confirmar solicitud'} <ArrowRight size={15} /></button><p className="mt-3 text-center text-[0.66rem] leading-5 text-[#77716a]">Si aún no has ingresado, te pediremos acceder antes de continuar.</p></form>
        </div>
      </div>
    </main>
  );
