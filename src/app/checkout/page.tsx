...
export default function CheckoutPage() {
  ...
  if (order) {
    ...
    return <main className="grid min-h-screen place-items-center bg-[#eee8de] px-4 pb-20 pt-28 text-[#171717]"><section className="w-full max-w-2xl border border-black/10 bg-[#f8f5ef] p-8 text-center shadow-[0_30px_80px_rgba(20,16,10,0.12)] sm:p-14"><span className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-[#171717] text-white"><Check size={23} /></span><p className="mt-7 text-[0.68rem] uppercase tracking-[0.32em] text-[#77716a]">Pedido {order.orderNumber}</p><h1 className="mt-4 font-serif text-5xl sm:text-6xl">Solicitud recibida.</h1><p className="mx-auto mt-5 max-w-lg text-sm leading-7 text-[#69635d]">Guardamos tu pedido en Mi ELEM. Ahora puedes revisar su avance y recibir la confirmación del equipo.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><Link href={`/mi-elem/pedidos/${order.id}`} className="inline-flex items-center justify-center gap-2 bg-[#171717] px-5 py-4 text-xs uppercase tracking-[0.18em] text-white">Ver mi pedido <ArrowRight size={14} /></Link><a href={whatsapp} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 border border-black/20 px-5 py-4 text-xs uppercase tracking-[0.18em]"><MessageCircle size={15} /> Coordinar por WhatsApp</a></div></section></main>;
  }
